...
class PublishedPost(models.Model):
    STATUS_CHOICES = [
        ('draft', 'Draft'),
        ('preview', 'Preview'),  # New status for editing
        ('published', 'Published'),
        ('failed', 'Failed'),
    ]
    ...
    edited_content = models.TextField(blank=True)  # New field for edited content
    html_content = models.TextField(blank=True)  # New field for final HTML
